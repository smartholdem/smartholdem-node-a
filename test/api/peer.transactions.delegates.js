...
describe('POST /peer/transactions', function () {
	...
	describe('registering a delegate', function () {
		...
		it('using undefined transaction.asset', function (done) {
			var transaction = node.sth.delegate.createDelegate(node.randomPassword(), node.randomDelegateName().toLowerCase());
		...
		describe('when account has funds', function () {
			...
			it('using invalid username should fail', function (done) {
				var transaction = node.sth.delegate.createDelegate(account.password, crypto.randomBytes(64).toString('hex'));
				transaction.fee = node.fees.delegateRegistrationFee;
			...
			describe('when lowercased username already registered', function () {
				it('using uppercase username should fail', function (done) {
					var transaction = node.sth.delegate.createDelegate(account2.password, account.username.toUpperCase());
		...
		describe('twice within the same block', function () {
			...
			it('should fail', function (done) {
				...
				var transaction = node.sth.delegate.createDelegate(account2.password, account2.username);

				account2.username = node.randomDelegateName().toLowerCase();
				var transaction2 = node.sth.delegate.createDelegate(account2.password, account2.username);
