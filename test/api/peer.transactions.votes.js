	...
	async.whilst(
		function () {
			...
			async.eachSeries(params.delegates, function (delegate, eachCb) {
				var transaction = node.sth.vote.createVote(params.passphrase, [params.action + delegate]);
...
function registerDelegate (account, done) {
	...
	var transaction = node.sth.delegate.createDelegate(account.password, account.username);
...
describe('POST /peer/transactions', function () {
	...
	it('using undefined transaction.asset', function (done) {
		var transaction = node.sth.vote.createVote(account.password, ['+' + delegate]);
	...
	it('voting for a delegate and then removing again within same block should fail', function (done) {
		node.onNewBlock(function (err) {
			...
			postVote(transaction, function (err, res) {
				...
				var transaction2 = node.sth.vote.createVote(account.password, ['-' + delegate]);
	...
	it('voting for 33 delegates at once should be ok', function (done) {
		node.onNewBlock(function (err) {
			var transaction = node.sth.vote.createVote(account.password, delegates.slice(0, 33).map(function (delegate) {
				return '+' + delegate;
			}));
	...
	it('voting for 34 delegates at once should fail', function (done) {
		node.onNewBlock(function (err) {
			var transaction = node.sth.vote.createVote(account.password, delegates.slice(0, 34).map(function (delegate) {
				return '+' + delegate;
			}));
...
describe('POST /peer/transactions after registering a new delegate', function () {
	...
	it('voting for self should be ok', function (done) {
		var transaction = node.sth.vote.createVote(account.password, ['+' + account.publicKey]);
	...
	it('exceeding maximum of 64 votes within same block should fail', function (done) {
		async.series([
			function (seriesCb) {
			...
			function (seriesCb) {
				...
				var transaction = node.sth.vote.createVote(account.password, slicedDelegates.map(function (delegate) {
					return '+' + delegate;
				}));

				postVote(transaction, function (err, res) {
					node.expect(res.body).to.have.property('success').to.be.not.ok;
					node.expect(res.body).to.have.property('error').to.equal('Maximum number of 64 votes exceeded (1 too many)');
					seriesCb();
				});
			}
		], function (err) {
