...
describe('GET /peer/transactions', function () {
	...
	it('using vendorField should be ok', function (done) {
		var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 1, "this is a test vendorfield", node.gAccount.password);
		memtx=transaction;
...
describe('POST /peer/transactions', function () {
	...
	it('using valid headers should be ok', function (done) {
		var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 1, null, node.gAccount.password);
	...
	// 	var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 2, null, node.gAccount.password);
	//
	// 	postTransaction(transaction, function (err, res) {
	// 		node.expect(res.body).to.have.property('success').to.be.ok;
	// 		node.expect(res.body).to.have.property('transactionIds');
	...
	it('using already confirmed transaction should succeed but return empty transactionIds property', function (done) {
		var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 3, null, node.gAccount.password);
	...
	it('using fees too low should fail', function (done) {
		...
		node.sth.crypto.sign(transaction, node.sth.crypto.getKeys(node.gAccount.password));
		transaction.id = node.sth.crypto.getId(transaction);
	...
	it('using transaction with undefined recipientId should fail', function (done) {
		var transaction = node.sth.transaction.createTransaction(undefined, 1, null, node.gAccount.password);
	...
	it('when sender has no funds should fail', function (done) {
		var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 1, null, 'randomstring');

		postTransaction(transaction, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.not.ok;
			node.expect(res.body).to.have.property('error').to.match(/Account does not have enough STH: [a-zA-Z0-9]+ balance: 0/);
			done();
		});
	});
	...
	it('when sender does not have enough funds should always fail', function (done) {
		...
		var transaction = node.sth.transaction.createTransaction(account.address, 1, null, node.gAccount.password);
		...
		postTransaction(transaction, function (err, res) {
			...
			node.onNewBlock(function () {
				var count = 1;
				var transaction2 = node.sth.transaction.createTransaction(node.gAccount.address, 2, null, account.password);

				node.async.doUntil(function (next) {
					postTransaction(transaction2, function (err, res) {
						node.expect(res.body).to.have.property('success').to.be.not.ok;
						node.expect(res.body).to.have.property('error').to.match(/Account does not have enough STH: [a-zA-Z0-9]+ balance: 1e-8/);
						count++;
						return next();
					});
				}, function () {
	...
	it('using very large amount and genesis block id should fail', function (done) {
		var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 100000000000000000, null, node.gAccount.password);
	...
	it('using overflown amount should fail', function (done) {
		var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 184819291270000000012910218291201281920128129, null,
	...
	it('using float amount should fail', function (done) {
		var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 1.3, null, node.gAccount.password);
