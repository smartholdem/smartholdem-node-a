...
function sendSTH (account, i, done) {
	var randomSTH = node.randomSTH();
	...
	node.put('/api/transactions/', {
		...
		if (res.body.success && i != null) {
			accounts[i].balance = randomSTH / node.normalizer;
		}
		done();
	});
}

function sendSTHFromMultisigAccount (amount, recipient, done) {
	node.put('/api/transactions/', {
...
before(function (done) {
	sendSTH(multisigAccount, null, done);
});
...
describe('PUT /api/api/transactions/', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendSTHFromMultisigAccount(100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
