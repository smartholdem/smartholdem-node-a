...
describe('GET /api/delegates', function () {
	it('using no params should be ok', function (done) {
		node.get('/api/delegates', function (err, res) {
			...
			node.expect(res.body.delegates).to.have.lengthOf(64);
	...
	it('using limit == 64 should be ok', function (done) {
		var limit = 64;
		var params = 'limit=' + limit;
	...
	it('using limit > 64 should fail', function (done) {
		var limit = 65;
		var params = 'limit=' + limit;

		node.get('/api/delegates?' + params, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.not.ok;
			node.expect(res.body).to.have.property('error').to.equal('Value 52 is greater than maximum 64');
			done();
		});
	});
...
describe('GET /api/delegates/count', function () {
	...
	it('should be ok', function (done) {
		node.get('/api/delegates/count', function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.ok;
			node.expect(res.body).to.have.property('count').to.be.at.least(64);
			done();
		});
	});
});
...
describe('GET /api/delegates/search', function () {
	...
	it('using criteria with length == 20 should be ok', function (done) {
		var q = 'gendelegate_123456789012'; // 20 characters
	...
	it('using criteria with length > 20 should fail', function (done) {
		var q = 'gendelegate_1234567890123'; // 21 characters
	...
	it('using critera == "gendelegate_1" should return 11 delegates', function (done) {
		var q = 'gendelegate_1';
	...
	it('using critera == "gendelegate_10" should return 1 delegates', function (done) {
		var q = 'gendelegate_10';
	...
	it('using critera == "gendelegate_64" should return 1 delegate', function (done) {
		var q = 'gendelegate_64';
	...
	it('using critera == "gendelegate_64" should have all properties', function (done) {
		var q = 'gendelegate_64';
	...
	it('using no limit should be ok', function (done) {
		...
		node.get('/api/delegates/search?q=' + q, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.ok;
			node.expect(res.body).to.have.property('delegates').that.is.an('array');
			node.expect(res.body.delegates).to.have.length(64);
			done();
		});
	});
	...
	it('using no orderBy should be ordered by ascending username', function (done) {
		...
		node.get('/api/delegates/search?q=' + q, function (err, res) {
			...
			node.expect(res.body.delegates).to.have.length(64);
			node.expect(res.body.delegates[0]).to.have.property('username');
			node.expect(res.body.delegates[0].username).to.equal('gendelegate_1');
			node.expect(res.body.delegates[24]).to.have.property('username');
			node.expect(res.body.delegates[24].username).to.equal('gendelegate_31');
			done();
		});
	});
	...
	it('using orderBy == "username:desc" should be ordered by descending username', function (done) {
		...
		node.get('/api/delegates/search?q=' + q + '&orderBy=username:desc', function (err, res) {
			...
			node.expect(res.body.delegates).to.have.length(64);
			node.expect(res.body.delegates[0]).to.have.property('username');
			node.expect(res.body.delegates[0].username).to.equal('gendelegate_9');
			node.expect(res.body.delegates[24]).to.have.property('username');
			node.expect(res.body.delegates[24].username).to.equal('gendelegate_33');
			done();
		});
	});
});
