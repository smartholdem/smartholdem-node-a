...
function sendSTH (params, done) {
	var transaction = node.sth.transaction.createTransaction(params.recipientId, params.amount, null, params.secret);
...
describe('POST /peer/transactions', function () {
	...
	describe('enabling second signature', function () {
		...
		it('using undefined transaction.asset', function (done) {
			var transaction = node.sth.signature.createSignature(node.randomPassword(), node.randomPassword());
		...
		describe('when account has funds', function () {
			...
			it('should be ok', function (done) {
				var transaction = node.sth.signature.createSignature(account.password, account.secondPassword);
	...
	describe('using second signature', function () {
		...
		it('when account does not have one should fail', function (done) {
			var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 1, null, node.gAccount.password, account.secondPassword);
		...
		it('using blank second passphrase should fail', function (done) {
			var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 1, null, account.password, '');
		...
		it('using fake second passphrase should fail', function (done) {
			var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 1, null, account.password, account2.secondPassword);
		...
		it('using valid second passphrase should be ok', function (done) {
			var transaction = node.sth.transaction.createTransaction('Sa9JKodiNeM7tbYjxwEhvvG1kBczhQxTN3', 1, null, account.password, account.secondPassword);
