...
// default db named
var db_name = "sth_" + network_name;

// optional premined accounts. Example :
// [
//   {address:"DH6VaL3ht6w4gHuXMsAq1xLe1UjbieEvxP", total:12300000000},
//   {address:"D5Z2byZnV2vAJRxYdxbgbUMGcPWossxDac", total:56700000000}
// ]
// total in satoshi
var genesisAccounts = [];
...
var totalpremine = 2100000000000000; // 21M
...
makeKeypair = function (seed) {
    return sthjs.crypto.getKeys(seed, networks[config.network]);
};
...
getBytes = function (block) {
    ...
    try {
        ...
        var payloadHashBuffer = new Buffer.from(block.payloadHash, 'hex');
        for (i = 0; i < payloadHashBuffer.length; i++) {
            bb.writeByte(payloadHashBuffer[i]);
        }

        var generatorPublicKeyBuffer = new Buffer.from(block.generatorPublicKey, 'hex');
        for (i = 0; i < generatorPublicKeyBuffer.length; i++) {
            bb.writeByte(generatorPublicKeyBuffer[i]);
        }

        if (block.blockSignature) {
            var blockSignatureBuffer = new Buffer.from(block.blockSignature, 'hex');
...
create = function (data) {
    ...
    for (var i = 0; i < transactions.length; i++) {
        ...
        var bytes = sthjs.crypto.getBytes(transaction);
...
var transactions = [];
var remainingfund = {};
sthjs.crypto.setNetworkVersion(networks[network_name].pubKeyHash);
...
premine.publicKey = sthjs.crypto.getKeys(premine.passphrase).publicKey;
premine.address = sthjs.crypto.getAddress(premine.publicKey, networks[config.network].pubKeyHash);

genesis.publicKey = sthjs.crypto.getKeys(genesis.passphrase).publicKey;
genesis.address = sthjs.crypto.getAddress(genesis.publicKey, networks[config.network].pubKeyHash);

// creation of delegates
for (var i = 1; i < delegates_count + 1; i++) {
    var delegate = {
        'passphrase': bip39.generateMnemonic(),
        'username': "delegate_" + i
    };

    delegate.publicKey = sthjs.crypto.getKeys(delegate.passphrase).publicKey;
    delegate.address = sthjs.crypto.getAddress(delegate.publicKey, networks[config.network].pubKeyHash);

    // create delegate
    var createDelegateTx = sthjs.delegate.createDelegate(delegate.passphrase, delegate.username);
    createDelegateTx.fee = 0;
    createDelegateTx.timestamp = 0;
    createDelegateTx.senderId = delegate.address;
    createDelegateTx.signature = sthjs.crypto.sign(createDelegateTx, sthjs.crypto.getKeys(delegate.passphrase));
    createDelegateTx.id = sthjs.crypto.getId(createDelegateTx);
...
for (var i = 0; i < genesisAccounts.length; i++) {
    ...
    //send SmartHoldem to account
    var premineTx = sthjs.transaction.createTransaction(account.address, account.total, null, premine.passphrase);

    premineTx.fee = 0;
    premineTx.timestamp = 0;
    premineTx.senderId = premine.address;
    premineTx.signature = sthjs.crypto.sign(premineTx, sthjs.crypto.getKeys(premine.passphrase));
    premineTx.id = sthjs.crypto.getId(premineTx);
...
var preminefund = sthjs.transaction.createTransaction(genesis.address, remainingfund.total, null, premine.passphrase);

preminefund.fee = 0;
preminefund.timestamp = 0;
preminefund.senderId = premine.address;
preminefund.signature = sthjs.crypto.sign(preminefund, sthjs.crypto.getKeys(premine.passphrase));
preminefund.id = sthjs.crypto.getId(preminefund);
transactions.push(preminefund);

var genesisBlock = create({
    keypair: sthjs.crypto.getKeys(genesis.passphrase, networks[config.network]),
...
for (var i = 0; i < delegates_count; i++) {
