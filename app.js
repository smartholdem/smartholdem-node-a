...
if(appConfig.network){
	...
	appConfig.network = networks.smartholdem;
}
	...
	async.auto({
		...
		connect: ['config', 'genesisblock', 'logger', 'build', 'network', function (scope, cb) {
			...
			scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
				scope.logger.info('# SmartHoldem node server started on: ' + scope.config.address + ':' + scope.config.port);

				if (!err) {
					if (scope.config.ssl.enabled) {
						scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
							scope.logger.info('SmartHoldem https started: ' + scope.config.ssl.options.address + ':' + scope.config.ssl.options.port);
